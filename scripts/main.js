...
console.log(Object.entries(webDev));
...
console.log(doesItContain("frame", webDev));
console.log(doesItContain("door", webDev));
...
function introduceTheActors(actorArray) {
    var introArray = [];
    for (var i = 0; i < 5; i++) {
        introArray.push(`Hi, I'm ${actors[i].name} and I was in ${actors[i].movie}.`);
    }
    return introArray;
}

console.log(introduceTheActors(actors));
  ...
  console.log(techCompanies.founderLookup("Apple"));
  console.log(techCompanies.founderLookup("Tesla"));
  ...
  function storeFounders(companyObject) {
      let founderArray = [];
      for (company in companyObject)
        founderArray.push(companyObject[company])
